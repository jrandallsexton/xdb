...
namespace XDB.Domains
{
    ...
    public class SqlDatabaseLayer
    {
        ...
        public string FilterString(string filterLogic, IList<XFilter> filters)
        {
            ...
            XPropertyDomain propLayer = new XPropertyDomain();
            XListValueDomain plvLayer = new XListValueDomain();
            ...
            foreach (XFilter filter in filters)
            {
                ...
                switch (prop.DataType)
                {
                    ...
                    case EDataType.User:
                        ...
                        propVal = new XUserDomain().DisplayValue(new Guid(filter.Value));
                ...
                foreach (PropertySelect reportProp in reportProps)
                {
                    ...
                    else
                    {
                            ...
                            else
                            {
                                ...
                                else if ((isInstance) && (!relation.IsInstance))
                                {
                                    ...
                                    if (!joinedTables.Contains(defTblNameRaw))
                                    {
                                        ...
                                        froms.AppendFormat("INNER JOIN [dbo].[{0}] ON [{1}].[AssetId] = [B].[{2}]", defTblNameRaw, defTblNameRaw, XPropertyIds.InstanceOf).AppendLine();
                                    }
                        ...
                        else
                        {
                            ...
                            if (reportProp.PropertyId.CompareTo(XPropertyIds.InstanceOf) == 0)
                            {
                                selects.AppendFormat("\t,[A].[{0}] AS [{1}]", lblDef, lblDef).AppendLine();
                            }
                            else if (reportProp.PropertyId.CompareTo(XPropertyIds.InstanceOfDesc) == 0)
                            {
                            ...
                            else if (reportProp.PropertyId.CompareTo(XPropertyIds.Description) == 0)
                            {
                                selects.AppendFormat("\t,[A].[{0}] AS [{1}]", reportProp.PropertyName, reportProp.PropertyName).AppendLine();
                            }
                            else if (reportProp.PropertyId.CompareTo(XPropertyIds.AssetType) == 0)
                            {
                                ...
                                if (!joinedTables.Contains(tempDefTableNameRaw))
                                {
                                    ...
                                    froms.AppendFormat("INNER JOIN [dbo].[{0}] ON [{1}].[AssetId] = [B].[{2}]", tempDefTableNameRaw, tempDefTableNameRaw, XPropertyIds.InstanceOfDesc).AppendLine();
                                }
                            ...
                            else if (reportProp.PropertyId.CompareTo(XPropertyIds.AssetName) == 0)
                            {
                        ...
                        if (reportProp.CustomReportFieldType == ECustomReportFieldType.NotApplicable)
                        {
                            ...
                            else if (reportProp.PropertyId.CompareTo(XPropertyIds.InstanceOfDesc) == 0)
                            {
                                ...
                                if (!joinedTables.Contains(tempTblName))
                                {
                                    ...
                                    froms.AppendFormat("INNER JOIN [dbo].[{0}] ON [{1}].[AssetId] = [B].[{2}]", tempTblName, tempTblName, XPropertyIds.InstanceOf).AppendLine();
                                }
                    ...
                    if (prop.IsSystem)
                    {
                        switch (prop.SystemType)
                        {
                            ...
                            case ESystemType.InstanceOf:
                                ...
                                //}
                                //wheres.Add(string.Format("[{0}].[{1}] {2} '{3}'", defTblNameRaw, Constants.PropertyIds.InstanceOf, f.Operator, f.Value));
                                wheres.Add(string.Format("[B].[{0}] {1} '{2}'", XPropertyIds.InstanceOf, f.Operator, f.Value));
                                break;
                            default:
                                break;
                        }
                                ...
                                if (subProp == null)
                                {
                                    switch (property.SystemType)
                                    {
                                        case ESystemType.InstanceOf:
                                            ...
                                            selectsRaw.AppendFormat("\t,[A].[InstanceOfId] AS [{0}]", XPropertyIds.InstanceOf).AppendLine();
                                            break;
                                        default:
