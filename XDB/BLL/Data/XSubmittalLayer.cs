...
namespace XDB.BLL
{
    ...
    public class XSubmittalLayer
    {

        private XSubmittalDal _dal = new XSubmittalDal();

        public XSubmittalLayer() { }

        public XSubmittalLayer(EApplicationInstance target)
        {
            var connString = Config.DbConnStringByInstance(target);
            this._dal = new XSubmittalDal(connString);
        }
        ...
        public XSubmittal PropertyValueSubmittal_Get(Guid id)
        {
            ...
            XPropertyLayer pLayer = new XPropertyLayer();
        ...
        public bool Save(XSubmittal submittal, bool isAutoApprove, Guid userId)
        {
            ...
            if (this._dal.PropertyValueSubmittal_Save(submittal))
            {

                if (new XValueLayer().PropertyValueList_Save(submittal.PropertyValues, userId))
                {
        ...
        private bool Approve(XSubmittal submittal, Guid userId)
        {
            ...
            if (submittal != null)
            {
                ...
                if (this.Save(submittal, false, userId))
                {
                        ...
                        if (prop != null)
                        {
                            ...
                            else if ((prop.DataType == EDataType.User) && (prop.RoleId.HasValue))
                            {
                                ...
                                if (!string.IsNullOrEmpty(pv.Value))
                                {
                                    ...
                                    {
                                        XRoleLayer roleLayer = new XRoleLayer();
                            ...
                            {
                                bool renamed = new XObjectLayer().Rename(pv.AssetId, pv.Value);
                            }
                            ...
                            if (relation != null)
                            {
                                ...
                                XObjectRelationDal relationDal = new XObjectRelationDal();
                    ...
                    return new XObjectLayer().MarkAsUpdated(submittal.AssetId, submittal.CreatedBy);
                }
            }
