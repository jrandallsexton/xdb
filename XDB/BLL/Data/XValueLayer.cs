...
namespace XDB.BLL
{
    ...
    public class XValueLayer
    {

        private XValueDal dal = new XValueDal();

        public XValueLayer() { }

        //public PropertyValueLayer(Core.Enumerations.EApplicationInstance target)
        //{
        //    string connString = Core.Config.SystemFrameworkHelper.DbConnStringByInstance(target);
        //    this.dal = new PropertyValueDal(connString);
        //}
        ...
        internal bool Save(XValue propertyValue)
        {
            XProperty prop = new XPropertyLayer().Get(propertyValue.PropertyId);

            return this.PropertyValueIsValid(prop, propertyValue) ? this.dal.Save(propertyValue) : false;
        }

        internal bool PropertyValue_Save(XValue propertyvalue, bool existingValueIsCaseSensitive)
        {
            XProperty prop = new XPropertyLayer().Get(propertyvalue.PropertyId);
            // since we're saving a single property value, we'll always delete the existing value
            return this.PropertyValueIsValid(prop, propertyvalue) ? this.dal.PropertyValue_Save(propertyvalue, true, existingValueIsCaseSensitive) : false;
        }

        public bool Delete(Guid id, Guid userId)
        {
            return this.dal.Delete(id, userId);
        }
        ...
        internal bool PropertyValueList_Save(List<XValue> values, Guid userId)
        {
            ...
            Dictionary<Guid, XProperty> properties = new XPropertyLayer().GetObjectDictionary(propIds);

            if (this.PropertyValuesAreValid(values, properties))
            {
                ...
                if (this.dal.PropertyValueList_Save(values, properties))
                {
        ...
        private bool PropertyValueIsValid(XProperty prop, XValue propertyValue)
        {
            ...
            if (new XUserLayer().ValidId(propertyValue.CreatedBy) == false)
            {
            ...
            if (prop.IsSystem)
            {
                switch (prop.SystemType)
                {
                    ...
                    case ESystemType.AssetType:
                        ...
                        {
                            return new XObjectTypeLayer().ValidId(newGuid);
                        }
                        return false;
                ...
                switch (prop.DataType)
                {
                    ...
                    case EDataType.URL:
                        if (!string.IsNullOrEmpty(propertyValue.Value))
                        {
                            ...
                            XUrl url = new XUrlLayer().Get(urlId);
                            if (url != null)
                            {
                                Uri testUrl;
                    ...
                    case EDataType.User:
                        ...
                        {
                            XUser member = new XUserLayer().Get(userId);
                            if (member == null)
                            {
        ...
        private bool PropertyValuesAreValid(List<XValue> propertyValues, Dictionary<Guid, XProperty> properties)
        {
        ...
        public Dictionary<Guid, string> PropertyValues_GetPotential(Guid assetTypeId, EAssetRequestType requestType, Guid propertyId)
        {
            ...
            PropertySelect rProp = new PropertySelect(Guid.NewGuid(), propertyId, new XPropertyLayer().DisplayValue(propertyId), 0);
        ...
        {
            return this.dal.PopulateDisplayValue(propertyValue.Value, new XPropertyLayer().Get(propertyValue.PropertyId));
        }
