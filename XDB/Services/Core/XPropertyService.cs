...
namespace XDB.API
{
    ...
    public class XPropertyService : XBaseService
    {

        public XPropertyService() : base(ECommonObjectType.XProperty) { }

        private XPropertyDomain bizLayer = new XPropertyDomain();
        ...
        {
            if (!new XUserService().HasPermission(userId, ECommonObjectType.XProperty, ESystemActionType.Delete))
            {
                throw new LogicalException(ExceptionMessages.PERM_PROP_DEL);
            }
            this.bizLayer.Delete(propertyId, userId);
        }
